# Exports AbsoluteReality (SD 1.5 finetune) + Hyper-SD 4-step LoRA to ONNX.
#
...
#
# Why AbsoluteReality:
#   - Lykon's photoreal SD 1.5 flagship (same shop as DreamShaper). Versatile
#     SFW workhorse — broader subject coverage than RV (which is portrait-
#     tuned), less stylized than DreamShaper, more general-purpose than
#     epiCRealism's environment lean. Fills the "SFW general workhorse"
#     niche cleanly.
#   - Notably less NSFW-leaning than Realistic Vision V6. Behaves much more
#     like DreamShaper / epiCRealism on the safety axis.
#   - Originally substituted in for XpucT/Deliberate, which is published as
#     a single-file CKPT on HF and lacks the diffusers folder layout that
#     optimum-cli requires. Lykon publishes AbsoluteReality directly in
#     diffusers format, same as DreamShaper.
#   - Ships with its own bundled VAE; no separate sd-vae-ft-mse pairing.
#
# Pipeline:
#   1. Load AbsoluteReality via diffusers (uses its bundled VAE)
...
# 4. Merge: load AbsoluteReality (which ships with its own VAE), fuse the
#    Hyper-SD-15 4-step LoRA into the UNet, save the fully-baked pipeline.
$mergedDir = Join-Path $env:TEMP "absolute-reality-hyper-merged-$([Guid]::NewGuid().ToString('N').Substring(0,8))"
Write-Host "Merging AbsoluteReality + Hyper-SD 4-step LoRA at $mergedDir ..." -ForegroundColor Cyan
...
print('Loading AbsoluteReality base (with its bundled VAE) ...')
...
# Same Hyper-SD-15 4-step LoRA used across SD 1.5 finetunes — base-
# architecture distillation, not finetune-specific.
