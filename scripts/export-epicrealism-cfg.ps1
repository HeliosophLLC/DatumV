# Exports epiCRealism (SD 1.5 finetune) to ONNX — NO distillation LoRA.
#
# This is the QUALITY counterpart to export-epicrealism-hyper.ps1. It is the
...
# UNet, which is driven by the epicrealism_cfg SQL body with classifier-free
# guidance over ~25 steps (see models/sql/epicrealism-cfg/2026-06-30.sql).
#
...
#
# epiCRealism (emilianJR) ships with its own VAE baked in, so there is no
# separate VAE pairing step (unlike Realistic Vision, which is noVAE).
#
# Pipeline:
#   1. Load epiCRealism via diffusers (uses its bundled VAE, no LoRA)
...
#
# Usage:
#   ./scripts/export-epicrealism-cfg.ps1
#       — exports to $env:DATUMV_MODELS\epicrealism-cfg-onnx (FP32)
#   ./scripts/export-epicrealism-cfg.ps1 -OutputDirectory C:\foo
...
param(
    ...
    [string]$OutputDirectory = $(
        if ($env:DATUMV_MODELS) {
            Join-Path $env:DATUMV_MODELS 'epicrealism-cfg-onnx'
        } else {
...
# 4. Load epiCRealism (with its bundled VAE) and save a non-distilled pipeline
#    that optimum-cli can export. No LoRA fusion here — that is the whole point
#    of this variant.
$mergedDir = Join-Path $env:TEMP "epicrealism-cfg-$([Guid]::NewGuid().ToString('N').Substring(0,8))"
Write-Host "Preparing epiCRealism (bundled VAE, no LoRA) at $mergedDir ..." -ForegroundColor Cyan
...
print('Loading epiCRealism base (with its bundled VAE) ...')
...
# at inference time by the epicrealism_cfg SQL body.
...
Write-Host ""
Write-Host "Next: place the export at <models root>\epicrealism-cfg\2026-06-30\ so the" -ForegroundColor DarkGray
Write-Host "      USING paths in sql/epicrealism-cfg/2026-06-30.sql resolve, then run that SQL." -ForegroundColor DarkGray
