# Exports epiCRealism (SD 1.5 finetune) + Hyper-SD 4-step LoRA to ONNX.
#
...
#
# Why epiCRealism instead of RV V6:
#   - epiCRealism produces photoreal-leaning outputs with broader subject
#     coverage than RV V6 (which is heavily portrait-tuned). Strong fit
#     for D&D environments, taverns, landscapes, group scenes — places
#     where RV's narrow portrait distribution starts to limit composition.
#   - Trained on a mix less NSFW-leaning than Realistic Vision V6.
#     Produces clothed characters and SFW outputs much more reliably.
#   - emilianJR ships the model with its own VAE baked in, so no separate
#     sd-vae-ft-mse pairing step is needed.
#
# Pipeline:
#   1. Load epiCRealism via diffusers (uses its bundled VAE)
...
# 4. Merge: load epiCRealism (which ships with its own VAE), fuse the
#    Hyper-SD-15 4-step LoRA into the UNet, save the fully-baked pipeline.
$mergedDir = Join-Path $env:TEMP "epicrealism-hyper-merged-$([Guid]::NewGuid().ToString('N').Substring(0,8))"
Write-Host "Merging epiCRealism + Hyper-SD 4-step LoRA at $mergedDir ..." -ForegroundColor Cyan
