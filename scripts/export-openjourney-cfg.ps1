# Exports Openjourney v4 (SD 1.5 finetune) to ONNX — NO distillation LoRA.
#
# This is the QUALITY counterpart to export-openjourney-hyper.ps1. It is the
...
# UNet, which is driven by the openjourney_cfg SQL body with classifier-free
# guidance over ~25 steps (see models/sql/openjourney-cfg/2026-06-30.sql).
#
...
#
# Openjourney v4 is a Midjourney-v4-style SD 1.5 fine-tune. Unlike v1/v3 it
# dropped the 'mdjrny-v4 style' trigger — prompt it like any modern fine-tune.
# It ships with its own VAE baked in, so there is no separate VAE pairing step.
#
# To build the more painterly v1 aesthetic instead, change the base id below
# from 'prompthero/openjourney-v4' to 'prompthero/openjourney' and re-run.
#
# Pipeline:
#   1. Load Openjourney v4 via diffusers (uses its bundled VAE, no LoRA)
...
#
# Usage:
#   ./scripts/export-openjourney-cfg.ps1
#       — exports to $env:DATUMV_MODELS\openjourney-cfg-onnx (FP32)
#   ./scripts/export-openjourney-cfg.ps1 -OutputDirectory C:\foo
...
param(
    ...
    [string]$OutputDirectory = $(
        if ($env:DATUMV_MODELS) {
            Join-Path $env:DATUMV_MODELS 'openjourney-cfg-onnx'
        } else {
...
# 4. Load Openjourney v4 (with its bundled VAE) and save a non-distilled
#    pipeline that optimum-cli can export. No LoRA fusion here — that is the
#    whole point of this variant.
$mergedDir = Join-Path $env:TEMP "openjourney-cfg-$([Guid]::NewGuid().ToString('N').Substring(0,8))"
Write-Host "Preparing Openjourney v4 (bundled VAE, no LoRA) at $mergedDir ..." -ForegroundColor Cyan
...
# at inference time by the openjourney_cfg SQL body.
...
Write-Host ""
Write-Host "Next: place the export at <models root>\openjourney-cfg\2026-06-30\ so the" -ForegroundColor DarkGray
Write-Host "      USING paths in sql/openjourney-cfg/2026-06-30.sql resolve, then run that SQL." -ForegroundColor DarkGray
