...
/// dispatch in that query, in arrival order. <see cref="OnChunk"/> may be
/// invoked many times per dispatch; <see cref="OnCompleted"/> fires exactly
/// once per dispatch on success, <see cref="OnFailed"/> exactly once on
/// throw.
/// </para>
/// <para>
...
/// without arena coordination. Chunks do not outlive the synchronous
/// <see cref="OnChunk"/> call — copy out anything the sink needs to keep.
/// </para>
/// <para>
...
public interface IModelStreamingSink
{
    ...
    void OnChunk(string modelName, ValueRef chunk);
    ...
    /// </summary>
    void OnCompleted(string modelName);
    ...
    /// </summary>
    void OnFailed(string modelName, Exception exception);
}
