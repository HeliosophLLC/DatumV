...
public sealed class HashSplitFunction : IScalarFunction
{
    ...
    public DataKind ValidateArguments(ReadOnlySpan<DataKind> argumentKinds)
    {
        ...
        if (!DataValueComparer.IsNumericScalar(argumentKinds[1]))
            throw new ArgumentException($"hash_split() seed (argument 2) must be numeric, got {argumentKinds[1]}.");
...
public sealed class RandomIntFunction : IScalarFunction
{
    ...
    public DataKind ValidateArguments(ReadOnlySpan<DataKind> argumentKinds)
    {
        ...
        if (!DataValueComparer.IsNumericScalar(argumentKinds[0]))
            throw new ArgumentException($"random_int() min (argument 1) must be numeric, got {argumentKinds[0]}.");

        if (!DataValueComparer.IsNumericScalar(argumentKinds[1]))
            throw new ArgumentException($"random_int() max (argument 2) must be numeric, got {argumentKinds[1]}.");
    ...
    public DataValue Execute(ReadOnlySpan<DataValue> arguments)
    {
        int min = (int)arguments[0].ToFloat();

        int max = (int)arguments[1].ToFloat();
...
public sealed class RandomRangeFunction : IScalarFunction
{
    ...
    public DataKind ValidateArguments(ReadOnlySpan<DataKind> argumentKinds)
    {
        ...
        if (!DataValueComparer.IsNumericScalar(argumentKinds[0]))
            throw new ArgumentException($"random_range() min (argument 1) must be numeric, got {argumentKinds[0]}.");

        if (!DataValueComparer.IsNumericScalar(argumentKinds[1]))
            throw new ArgumentException($"random_range() max (argument 2) must be numeric, got {argumentKinds[1]}.");
    ...
    public DataValue Execute(ReadOnlySpan<DataValue> arguments)
    {
        float min = arguments[0].ToFloat();

        float max = arguments[1].ToFloat();
...
public sealed class RandomNormalFunction : IScalarFunction
{
    ...
    public DataKind ValidateArguments(ReadOnlySpan<DataKind> argumentKinds)
    {
        ...
        if (!DataValueComparer.IsNumericScalar(argumentKinds[0]))
            throw new ArgumentException($"random_normal() mean (argument 1) must be numeric, got {argumentKinds[0]}.");

        if (!DataValueComparer.IsNumericScalar(argumentKinds[1]))
            throw new ArgumentException($"random_normal() stddev (argument 2) must be numeric, got {argumentKinds[1]}.");
...
public sealed class RandomBooleanFunction : IScalarFunction
{
    ...
    public DataKind ValidateArguments(ReadOnlySpan<DataKind> argumentKinds)
    {
        ...
        if (!DataValueComparer.IsNumericScalar(argumentKinds[0]))
            throw new ArgumentException($"random_boolean() probability must be numeric, got {argumentKinds[0]}.");
    ...
    public DataValue Execute(ReadOnlySpan<DataValue> arguments)
    {
        float probability = arguments[0].ToFloat();
