...
public sealed class VecSliceFunction : IScalarFunction
{
    ...
    public DataKind ValidateArguments(ReadOnlySpan<DataKind> argumentKinds)
    {
        ...
        if (!DataValueComparer.IsNumericScalar(argumentKinds[1]))
            throw new ArgumentException("vec_slice() second argument (start) must be numeric.");
        if (!DataValueComparer.IsNumericScalar(argumentKinds[2]))
            throw new ArgumentException("vec_slice() third argument (length) must be numeric.");
...
public sealed class VecFunction : IScalarFunction
{
    ...
    public DataKind ValidateArguments(ReadOnlySpan<DataKind> argumentKinds)
    {
        ...
        {
            if (!DataValueComparer.IsNumericScalar(argumentKinds[i]) && argumentKinds[i] != DataKind.Vector)
                throw new ArgumentException($"vec() argument {i + 1} must be numeric or Vector.");
        }
        return DataKind.Vector;
    }
...
public sealed class VecPadFunction : IScalarFunction
{
    ...
    public DataKind ValidateArguments(ReadOnlySpan<DataKind> argumentKinds)
    {
        ...
        if (!DataValueComparer.IsNumericScalar(argumentKinds[1]))
            throw new ArgumentException("vec_pad() second argument (targetLength) must be numeric.");
        if (!DataValueComparer.IsNumericScalar(argumentKinds[2]))
            throw new ArgumentException("vec_pad() third argument (fillValue) must be numeric.");
        return DataKind.Vector;
    }
...
public sealed class VecRepeatFunction : IScalarFunction
{
    ...
    public DataKind ValidateArguments(ReadOnlySpan<DataKind> argumentKinds)
    {
        ...
        if (!DataValueComparer.IsNumericScalar(argumentKinds[1]))
            throw new ArgumentException("vec_repeat() second argument (count) must be numeric.");
        return DataKind.Vector;
    }
    ...
    public DataKind ValidateArguments(ReadOnlySpan<DataKind> argumentKinds)
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            if (!DataValueComparer.IsNumericScalar(argumentKinds[i]))
                throw new ArgumentException($"linspace() argument {i + 1} must be numeric.");
        }
        return DataKind.Vector;
    }
...
public sealed class ArangeFunction : IScalarFunction
{
    ...
    public DataKind ValidateArguments(ReadOnlySpan<DataKind> argumentKinds)
    {
        ...
        {
            if (!DataValueComparer.IsNumericScalar(argumentKinds[i]))
                throw new ArgumentException($"arange() argument {i + 1} must be numeric.");
        }
        return DataKind.Vector;
    }
