...
/// <summary>
/// <c>image_decode(bytes Array&lt;UInt8&gt;) → Image</c>. Wraps raw encoded
/// image bytes (PNG / JPEG / WebP / BMP / TIFF, anything SkiaSharp recognises)
/// as a typed <c>Image</c> value so subsequent image functions
/// (<c>image_width</c>, <c>image_height</c>, <c>image_crop</c>, …) can consume
/// it directly. The proximate use case is SQL recipes that compose
...
public sealed class ImageDecodeFunction : IFunction, IScalarFunction
{
    ...
    public static string Description =>
        "Wraps a raw encoded-image byte array as a typed Image value: "
        + "image_decode(bytes Array<UInt8>) → Image. No pixel decoding — bytes pass "
