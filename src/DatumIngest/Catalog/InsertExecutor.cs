        ...
        for (int rowIndex = 0; rowIndex < values.Rows.Count; rowIndex++)
        {
            ...
            for (int targetIndex = 0; targetIndex < targetSchema.Columns.Count; targetIndex++)
            {
                ...
                if (sourceIndex >= 0)
                {
                    ...
                    {
                        // Explicit value supplied. GENERATED ALWAYS AS
                        // IDENTITY (and legacy bare IDENTITY) rejects
                        // explicit values per row — the only way to fill
                        // an ALWAYS IDENTITY slot from a column-listed
                        // VALUES row is the `DEFAULT` keyword (handled
                        // above). GENERATED BY DEFAULT accepts as-is.
