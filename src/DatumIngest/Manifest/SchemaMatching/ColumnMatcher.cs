...
internal static class ColumnMatcher
{
    ...
    /// <summary>
    /// Minimum estimated distinct count for a Categorical column to be considered
    /// as a join key in Categorical↔Categorical pairing. Columns below this floor
    /// are flags or low-cardinality codes that match too many rows to form a
    /// meaningful equi-join. Identifier and ForeignKey columns bypass this check.
    /// </summary>
    private const long MinimumCategoricalJoinDistinctCount = 20;
    ...
    internal static bool IsRolePairJoinable(FeatureManifest left, FeatureManifest right)
    {
        ...
        // Both Categorical → allowed only when both sides have enough distinct
        // values to be plausible join keys and their domain sizes are close.
        if (leftValue is ColumnRole.Categorical && rightValue is ColumnRole.Categorical)
        {
            ...
            // Reject low-cardinality categoricals that cannot discriminate rows.
            if (leftNdv < MinimumCategoricalJoinDistinctCount ||
                rightNdv < MinimumCategoricalJoinDistinctCount)
            {
                return false;
            }
