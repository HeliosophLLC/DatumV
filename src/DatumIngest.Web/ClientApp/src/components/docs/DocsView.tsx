...
import { Check, ChevronDown, ChevronRight, Copy, FileText, Folder, Search } from 'lucide-react';
...
/**
 * Override for the markdown `<pre>` block. Wraps the original `<pre>` in
 * a positioned container so a Copy button can sit pinned to the top-right
 * corner without disturbing the code's layout. Reads the rendered text via
 * the `<pre>` ref's `innerText` on click — robust against the highlight.js
 * `<span class="hljs-*">` token wrapping that rehype-highlight inserts.
 */
function CodeBlock({
  children,
  ...rest
