...
| **Lakehouse table format** (ingest WAL → snapshots → delete vectors) | XL | Phased evolution toward Iceberg/Delta-shaped semantics: ingest journaling for crash safety (Phase 0), journaled DDL — `CREATE`/`DROP`/rename — over the same WAL (Phase 1), snapshot manifest enabling time travel and dataset versioning (Phase 2), delete vectors enabling row-level `DELETE`/`UPDATE` (Phase 3). Phases 0–2 directly support ML-pipeline use cases (reproducibility, training-set freezes, audit); Phase 3 is where DML completes. Subsumes parts of "Primary key persistence" and "Datum File Compaction" — see Lakehouse Evolution notes below. **Not currently planned** — captured for design coherence so smaller WAL work doesn't have to be rewritten if later phases ever become priorities |
...
**Lakehouse Evolution**: Four-phase plan to evolve from append-only ingest to lakehouse table-format semantics (Iceberg/Delta-shaped). Each phase is independently shippable and useful on its own; later phases add capability without invalidating earlier ones. Captured for design coherence so the smaller Phase 0 work doesn't paint future phases into a corner.

*Phase 0 — Ingest WAL*: Append-only journal at a configurable directory (typical deployment: dedicated drive, separate from datum reads and spill writes) recording per-ingest lifecycle stages (`Started`, `RowGroupWritten`, `FooterWritten`, `SidecarWritten`, `CatalogUpdated`, `Committed`/`Failed`/`RolledBack`). Crash recovery scans for non-terminal records and best-effort cleans up partial outputs. Source-hash dedup as a side benefit (`Begin` rejects re-ingest of an already-`Committed` source). Records are fixed-stride structs with per-record CRC32 trailers so torn writes are detectable. New types: `IngestJournal`, `IngestRecord`, `IngestRecovery`, `IngestStage`. fsync-bound, tiny throughput — well-suited to its own spindle.

*Phase 1 — Journaled DDL*: Extend the WAL to record catalog mutations: `CREATE TABLE`, table rename, `DROP TABLE`, schema alteration. Catalog updates become atomic with crash safety. The catalog itself becomes a fold over the WAL (or a periodic checkpoint snapshot + tail of recent records). New WAL record kinds: `TableCreated`, `TableRenamed`, `TableDropped`, `SchemaAltered`. DDL gets the same crash-safe semantics as ingest.

*Phase 2 — Snapshot manifest*: Catalog evolves from "table T points at file path F" to "table T at snapshot N consists of files [F1, F2, …]". `INSERT` writes a new `.datum` file then atomically advances the snapshot pointer in the WAL. Readers pin a snapshot at query start (transaction-equivalent isolation). Falls out: time travel (`AS OF SNAPSHOT n` syntax), dataset versioning (training-set freezes — pin snapshot N, train against it forever), audit ("what did this query see when it ran?"). **Highest leverage for the ML-from-SQL strategic direction**: reproducibility and training-set immutability are first-class. New types: `SnapshotManifest`, `SnapshotId`, `TableVersion`. Optional ALTER TABLE schema evolution: each snapshot carries its own schema; readers project to the requested schema.

*Phase 3 — Delete vectors*: Sidecar bitmap files (`.datum-deletes`) per data file recording row-level deletes. Readers consult the bitmap and skip flagged rows (merge-on-read). `DELETE FROM t WHERE …` writes a delete-vector update across affected files; `UPDATE` = paired DELETE + INSERT in the same transaction. Compaction merges accumulated deletes back into rewritten data files (subsumes "Datum File Compaction"'s tombstone/orphan logic). New types: `DeleteVectorWriter`, `DeleteVectorReader`, `MergeOnReadFilter`. This is where DML actually completes.

*What's still missing after Phase 3*: Multi-writer concurrent INSERT against the same table needs optimistic concurrency control on the snapshot pointer — Iceberg/Delta both have this; it's the part where lakehouse formats get genuinely complicated (retries, conflict detection, isolation levels). Single-writer-per-table or coordinator-serialized writes are the no-OCC starting point and cover most analytical/ML workloads.

*Boundary with current roadmap items*: "Primary key persistence" already proposes shard-aware INSERT and PK validation across shards — fits naturally as Phase 2/3 work once snapshots exist. "Datum File Compaction" is the operational counterpart to Phase 3's delete-vector compaction. "Persistent views" composes with Phase 2 — views are queries against named snapshots.

*Why under consideration, not committed*: The product priority is ML-from-SQL, not building a full database. Phases 0–2 align well with that priority (reproducibility, dataset versioning, audit); Phase 3 (full DML) does not — analytical/ML workloads typically rebuild tables rather than mutate them. Documenting all four phases now keeps any near-term ingest-WAL work design-coherent.
