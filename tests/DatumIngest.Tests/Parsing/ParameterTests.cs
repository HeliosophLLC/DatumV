...
public sealed class ParameterTests
{
    ...
    [InlineData("TRUE")]
    [InlineData("True")]
    public void ValueParser_ParsesTrue(string input)
    {
        DataValue result = ParameterValueParser.Parse(input);
    ...
    [InlineData("FALSE")]
    [InlineData("False")]
    public void ValueParser_ParsesFalse(string input)
    {
        DataValue result = ParameterValueParser.Parse(input);
        Assert.Equal(DataKind.Boolean, result.Kind);
    ...
    [InlineData("NULL")]
    [InlineData("Null")]
    public void ValueParser_ParsesNull(string input)
    {
        DataValue result = ParameterValueParser.Parse(input);
    ...
    [InlineData("123abc")]
    [InlineData("")]
    public void ValueParser_ParsesStrings(string input)
    {
        DataValue result = ParameterValueParser.Parse(input);
    ...
    [Fact]
    public void CliOptions_ParsesParamFlag()
    {
        string[] args = ["query", "SELECT * FROM t", "--source", "csv:t=t.csv",
            "--param", "threshold=0.5"];

        CliOptions options = CliOptions.Parse(args);

        Assert.Single(options.Parameters);
        Assert.Equal("0.5", options.Parameters["threshold"]);
    }

    [Fact]
    public void CliOptions_ParsesMultipleParams()
    {
        string[] args = ["query", "SELECT * FROM t", "--source", "csv:t=t.csv",
            "--param", "threshold=0.5", "--param", "name=hello"];

        CliOptions options = CliOptions.Parse(args);

        Assert.Equal(2, options.Parameters.Count);
        Assert.Equal("0.5", options.Parameters["threshold"]);
        Assert.Equal("hello", options.Parameters["name"]);
    }

    [Fact]
    public void CliOptions_ThrowsOnMissingParamValue()
    {
        string[] args = ["query", "SELECT * FROM t", "--source", "csv:t=t.csv",
            "--param"];
    ...
    [Fact]
    public void CliOptions_ThrowsOnInvalidParamFormat()
    {
        string[] args = ["query", "SELECT * FROM t", "--source", "csv:t=t.csv",
            "--param", "noequalssign"];
    ...
    [Fact]
    public void CliOptions_ParamValueContainingEquals()
    {
        string[] args = ["query", "SELECT * FROM t", "--source", "csv:t=t.csv",
            "--param", "expr=a=b"];
        ...
        Assert.Equal("a=b", options.Parameters["expr"]);
    }
