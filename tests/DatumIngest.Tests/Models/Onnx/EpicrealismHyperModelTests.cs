...
/// <summary>
/// Smoke tests for the epiCRealism + Hyper-SD registration. Same SD 1.5
...
public sealed class EpicrealismHyperModelTests : ServiceTestBase
{
    private static string ModelDirectory => Path.Combine(
        ModelCatalog.DefaultModelDirectory, BuiltinModels.EpicrealismHyperFolder);

    private static string AnchorFilePath => Path.Combine(
        ModelCatalog.DefaultModelDirectory, BuiltinModels.EpicrealismHyperAnchor);

    private static bool ModelAvailable => File.Exists(AnchorFilePath);

    [Fact]
    public void Load_EpicrealismHyper_ExposesExpectedSignature()
    {
        if (!ModelAvailable) return;

        using StableDiffusionTurboModel model = new(name: "epicrealism_hyper", modelDirectory: ModelDirectory);

        Assert.Equal("epicrealism_hyper", model.Name);
    ...
    public async Task InferBatch_SimplePrompt_ReturnsValid512x512Image()
    {
        ...
        using StableDiffusionTurboModel model = new(
            name: "epicrealism_hyper",
            modelDirectory: ModelDirectory,
            seed: 42);

        DatumIngest.Functions.ValueRef[][] inputs =
            [[DatumIngest.Functions.ValueRef.FromString("a bustling medieval tavern at night, warm hearth glow, photoreal")]];
    ...
    public async Task InferBatch_TwoDifferentPrompts_ProducesTwoDistinctImages()
    {
        ...
        DatumIngest.Functions.ValueRef[][] inputs =
        [
            [DatumIngest.Functions.ValueRef.FromString("a windswept mountain pass at dawn, golden light, photoreal")],
            [DatumIngest.Functions.ValueRef.FromString("a misty swamp with crooked trees, atmospheric, photoreal")],
        ];
    ...
    [Fact]
    public void Catalog_RegisterAndResolve_YieldsEpicrealismHyperModel()
    {
        if (!ModelAvailable) return;

        ModelCatalog catalog = new(modelDirectory: ModelCatalog.DefaultModelDirectory);
        BuiltinModels.RegisterEpicrealismHyper(catalog);

        ModelCatalogEntry? entry = catalog.TryGetEntry("epicrealism_hyper");
        Assert.NotNull(entry);
        Assert.Equal("onnx", entry!.Backend);
        Assert.Equal(BuiltinModels.EpicrealismHyperAnchor, entry.RelativePath);
        ...
        using ModelLease lease = catalog.ResolveLeaseSynchronously("epicrealism_hyper");
