...
public class UdfValidationTests : ServiceTestBase
{
    ...
    public async Task NotNullParam_NullArg_Throws()
    {
        ...
        IQueryPlan plan = catalog.Plan("SELECT udf.shout(v) FROM data");
    ...
    public async Task ReturnsType_CoercesBodyToDeclaredKind()
    {
        ...
        IQueryPlan plan = catalog.Plan("SELECT udf.truncated(v) FROM data");
    ...
    public async Task ReturnsTypeIsNotNull_NullBody_Throws()
    {
        ...
        catalog.Plan(
            ...
            "AS try_cast(@s, INT32)");
        IQueryPlan plan = catalog.Plan("SELECT udf.parsed(s) FROM data");
