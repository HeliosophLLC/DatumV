...
The pass is implemented by [`InlineAccessorElider`](../../src/DatumV/Execution/InlineAccessorElider.cs) and runs during [`QueryPlanner.Finalize`](../../src/DatumV/Execution/QueryPlanner.cs), between [`ImageMetadataLowerer`](../../src/DatumV/Execution/ImageMetadataLowerer.cs) and [`CommonSubexpressionEliminator`](common-subexpression-elimination.md). The upstream lowerer rewrites composite image-metadata calls (`pixel_count`, `dimensions(img, literal)`) into compositions of the elidable accessors below, so a single planner pipeline turns both the primitive accessors and their composites into struct-byte reads.
...
A function is eligible for elision when it implements [`IInlineMetadataAccessor`](../../src/DatumV/Functions/IInlineMetadataAccessor.cs). The marker is a one-property interface — `InlineAccessorField Field { get; }` — that names which inline-metadata field the function reads. Implementing it is an opt-in choice by the function author: the function continues to ship its original `ExecuteAsync` body, which the evaluator delegates back to whenever the inline metadata isn't available (the **fallback** path described below).
...
The resolved instance's `Field` value identifies which inline-metadata field to read; the [`InlineAccessorDescriptors`](../../src/DatumV/Execution/InlineAccessorField.cs) static table maps each field to the expected argument kind, the result kind, and the canonical SQL function name used to recover the fallback function at evaluation time.
...
3. **Add an [`InlineAccessorField`](../../src/DatumV/Execution/InlineAccessorField.cs) entry** and a descriptor row with the argument kind, result kind, and the SQL function name.
