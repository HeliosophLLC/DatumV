...
FROM specifies where your data comes from. Besides regular tables, you can query subqueries as inline tables, or use table-valued functions like `range` and `generate_series` to generate data on the fly — useful for creating sequences, synthetic test data, or cross-joining with computed values.
