...
`DEFAULT` and `GENERATED ALWAYS AS (expr)` (computed) are both accepted on `ADD COLUMN`:
...
`NOT NULL` on `ADD COLUMN` is still rejected — there's no way to materialise a non-null value for existing rows without a user-supplied backfill expression. Use `ADD COLUMN col Kind DEFAULT <expr>` or `ADD COLUMN col Kind AS (expr)` instead.
