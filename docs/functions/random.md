...
`random_int(min, max)` → Float32

Random integer in [min, max] (both inclusive).

### random_range

`random_range(min, max)` → Float32
...
`random_permutation(length)` → Vector

Random permutation of [0, length) via Fisher-Yates.

### random_choice

`random_choice(array, count)` → Array
