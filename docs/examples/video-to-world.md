...
The query below is the whole thing. It reads the video, runs
`depth_anything_v3_large_full` on every sampled frame (metric depth,
per-pixel confidence, and the camera intrinsics — all from one forward
pass), chains frame-to-frame poses into a trajectory, unprojects each frame
into a world-space point cloud, and aggregates two outputs: the fused world
and the movie.

```sql
DECLARE model_in_w Float32 = 518.0::Float32  -- DA-V3 native input width
...
WITH RECURSIVE frames AS MATERIALIZED (
    ...
    SELECT
        ...
        LET img_depth = models.depth_anything_v3_large_full(img),
        ROW_NUMBER() OVER (ORDER BY vid.frame_index) - 1 AS seq,
        img,
        img_depth['intrinsics'] k,           -- 3x3 K, already in img pixel coordinates
        img_depth['depth'] depth,            -- metres, already source-aligned
        img_depth['confidence'] confidence   -- per-pixel reliability, source-aligned
    FROM video_unnest_frames(video_path, 0, 4) vid
),
k0 AS MATERIALIZED (
    -- One camera, one K: pin the first frame's predicted intrinsics for
    -- the whole video. The model re-estimates K every frame with a little
    -- jitter, but the physical camera never changed — and the SAME K must
    -- drive both pose recovery and unprojection, or the geometry warps
    -- under rotation.
    SELECT k FROM frames WHERE seq = 0
),
curr_prev AS MATERIALIZED (
    ...
    FROM frames f1
    CROSS JOIN k0
    LEFT JOIN frames f2 ON f2.seq = (f1.seq - 1)
),
...
- **The world warps or the floor tilts when the video turns** — the poses
  ...
  single pinned `k0.k` flowing to *both* `pose_from_rgbd` and
  `point_cloud_from_depth_pinhole_intrinsics_with_confidence`, exactly as
  in the script.
