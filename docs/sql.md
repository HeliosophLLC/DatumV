...
DatumIngest supports a subset of SQL designed for ML dataset ETL: SELECT, SELECT DISTINCT, FROM, JOIN, WHERE, GROUP BY, HAVING, window functions (OVER/PARTITION BY), INTO, ORDER BY, LIMIT, OFFSET, subqueries, and Common Table Expressions (WITH / WITH RECURSIVE).
