-- ============================================================================
-- epiCRealism + CFG — text-to-image, CreativeML OpenRAIL-M.
-- ============================================================================
--
-- Catalog id:  epicrealism-cfg                 (models/catalog.json)
-- ONNX files:  text_encoder/model.onnx, unet/model.onnx, vae_decoder/model.onnx
-- License:     CreativeML OpenRAIL-M
-- Upstream:    https://huggingface.co/emilianJR/epiCRealism (ships its own VAE)
--
-- The QUALITY counterpart to epicrealism-hyper. Same SD 1.5 base, but this
-- body exports the model **without** the Hyper-SD distillation LoRA and drives
-- it with classifier-free guidance (CFG) over a standard ~25-step schedule.
-- That recovers the prompt adherence, detail, and contrast the 4-step Hyper
-- variant trades away for speed — at the cost of two UNet evaluations per step
-- and roughly an order of magnitude more steps.
--
-- Differs from the Hyper bodies (see
-- models/sql/epicrealism-hyper/2026-05-29.sql) in three places:
...
CREATE OR REPLACE MODEL epicrealism_cfg(
  prompt String,
  negative_prompt String = ''
    COMMENT 'Things to steer away from — e.g. ''blurry, lowres, distorted perspective, watermark, oversaturated''. Realized through the classifier-free guidance term, so it only has effect when guidance > 1. Leave empty for a plain unconditional baseline.',
  steps  Int32 = 25
...
USING 'epicrealism-cfg/2026-06-30/text_encoder/model.onnx' AS text_encoder,
      'epicrealism-cfg/2026-06-30/unet/model.onnx'         AS unet,
      'epicrealism-cfg/2026-06-30/vae_decoder/model.onnx'  AS vae_decoder
AS BEGIN
