...
CREATE OR REPLACE MODEL sd_turbo(
  ...
  seed   Int64 = NULL
    COMMENT 'Optional RNG seed for the latent noise. Leave unset (NULL) for a fresh random image on every call; pass a fixed integer to reproduce the same image for a given prompt, steps, and size. Seeds both the initial latent and each step''s ancestral renoising (via seed + step index), so a fixed seed reproduces the whole trajectory on a fixed execution provider; output is not bit-identical to other diffusion tools and GPU runs may still vary slightly.'
) RETURNS Image
...
AS BEGIN
  ...
  BEGIN
    ...
    -- seed + i gives each step a distinct deterministic noise stream; when
    -- seed is NULL the arithmetic is NULL too, falling back to random.
    SET latents = array_axpy(
      latents, sigma_up, sample_normal(4 * latent_dim * latent_dim, seed + i));
    SET i = i + 1
  END;
