-- ============================================================================
-- DreamShaper 8 + CFG — text-to-image, CreativeML OpenRAIL-M.
-- ============================================================================
--
-- Catalog id:  dreamshaper-cfg                 (models/catalog.json)
-- ONNX files:  text_encoder/model.onnx, unet/model.onnx, vae_decoder/model.onnx
-- License:     CreativeML OpenRAIL-M
-- Upstream:    https://huggingface.co/Lykon/dreamshaper-8 (ships its own VAE)
--
-- The QUALITY counterpart to dreamshaper-hyper. Same SD 1.5 base, but this
...
-- models/sql/dreamshaper-hyper/2026-05-29.sql) in three places:
...
CREATE OR REPLACE MODEL dreamshaper_cfg(
  prompt String,
  negative_prompt String = ''
    COMMENT 'Things to steer away from — e.g. ''blurry, lowres, bad anatomy, deformed, watermark, extra fingers''. Realized through the classifier-free guidance term, so it only has effect when guidance > 1. Leave empty for a plain unconditional baseline.',
  steps  Int32 = 25
...
USING 'dreamshaper-cfg/2026-06-30/text_encoder/model.onnx' AS text_encoder,
      'dreamshaper-cfg/2026-06-30/unet/model.onnx'         AS unet,
      'dreamshaper-cfg/2026-06-30/vae_decoder/model.onnx'  AS vae_decoder
AS BEGIN
