-- ============================================================================
-- Openjourney v4 + CFG — text-to-image, CreativeML OpenRAIL-M.
-- ============================================================================
--
-- Catalog id:  openjourney-cfg                 (models/catalog.json)
-- ONNX files:  text_encoder/model.onnx, unet/model.onnx, vae_decoder/model.onnx
-- License:     CreativeML OpenRAIL-M
-- Upstream:    https://huggingface.co/prompthero/openjourney-v4 (ships its own VAE)
--
-- The QUALITY counterpart to openjourney-hyper. Same SD 1.5 base (Openjourney
-- v4, a Midjourney-v4-style fine-tune), but this body exports the model
-- **without** the Hyper-SD distillation LoRA and drives it with classifier-free
-- guidance (CFG) over a standard ~25-step schedule. That recovers the prompt
-- adherence, detail, and contrast the 4-step Hyper variant trades away for
-- speed — at the cost of two UNet evaluations per step and roughly an order of
-- magnitude more steps.
--
-- No activator phrase: Openjourney v4 dropped the 'mdjrny-v4 style' trigger
-- that v1/v3 required. Prompt it like any modern fine-tune.
--
-- Differs from the Hyper bodies (see
-- models/sql/openjourney-hyper/2026-05-29.sql) in three places:
...
--
-- Canonical annotated CFG reference:
-- models/sql/realistic-vision-cfg/2026-06-30.sql — this body is identical
-- except for the USING paths and the model name.
-- ============================================================================

CREATE OR REPLACE MODEL openjourney_cfg(
  prompt String,
  negative_prompt String = ''
    COMMENT 'Things to steer away from — e.g. ''blurry, lowres, deformed, bad anatomy, watermark, oversaturated''. Realized through the classifier-free guidance term, so it only has effect when guidance > 1. Leave empty for a plain unconditional baseline.',
  steps  Int32 = 25
...
USING 'openjourney-cfg/2026-06-30/text_encoder/model.onnx' AS text_encoder,
      'openjourney-cfg/2026-06-30/unet/model.onnx'         AS unet,
      'openjourney-cfg/2026-06-30/vae_decoder/model.onnx'  AS vae_decoder
AS BEGIN
