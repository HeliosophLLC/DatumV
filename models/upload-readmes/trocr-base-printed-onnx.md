...
Toolchain: `torch 2.4.x` (CUDA 12.4), `transformers 4.45.2`, `optimum[onnxruntime] 1.24.0`, `onnxconverter-common>=1.14`. Full conversion script: [`scripts/export-trocr-base-printed-fp16.ps1`](https://github.com/HeliosophLLC/DatumV/blob/main/scripts/export-trocr-base-printed-fp16.ps1) in the Heliosoph repo (despite the `-fp16` suffix on the script name, it produces both precisions in one run).
