WITH fine_labels AS (
    ...
    FROM open_archive($archive, 'cifar-100-binary/fine_label_names.txt') tt
...
coarse_labels AS (
    SELECT
        CAST(row_number() OVER () - 1 AS UInt8) AS label_id,
        f.fields[1] AS label_name
    FROM open_archive($archive, 'cifar-100-binary/coarse_label_names.txt') tt
    JOIN read_csv(tt.bytes) f
)
SELECT
    c.idx AS idx,
...
FROM open_archive($archive, 'cifar-100-binary/train.bin') AS a
